...
┌─────────────────────────────────────────────────────────┐
│                      Clients                             │
│     TUI (Ink)    │    Web UI    │   Desktop (Electron)   │
└──────────────────────┬──────────────────────────────────┘
                       │  HTTP + SSE
┌──────────────────────▼──────────────────────────────────┐
│                 @codingcode/core                         │
│                                                          │
│  ┌─────────────────┐  ┌──────────────┐  ┌────────────┐  │
│  │   ReAct Loop    │  │  Tool System │  │  Subagent   │  │
│  │  (纯手写引擎)    │  │  (可扩展注册) │  │  (Worker池) │  │
│  └────────┬────────┘  └──────┬───────┘  └─────┬──────┘  │
│           │                  │                 │          │
│  ┌────────▼──────────────────▼─────────────────▼──────┐  │
...
| `~/.codingcode/rules.md` + `./.coderules` | 全局 + 项目级规则，注入 system prompt |
...
| **Debugger** | read_file, list_dir, execute_command, search_code | 20 | 只读调试，不修改文件 |
| **Reviewer** | read_file, list_dir, search_code | 10 | 纯代码审查，无命令执行权 |
...
./.coderules                  # 项目级规则，自动注入 system prompt
```
...
| 环节 | 配置方式 |
|---|---|
| System Prompt | 角色文件 (`prompts/`) + 规则文件 (`rules.md` / `.coderules`) |
