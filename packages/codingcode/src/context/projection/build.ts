...
export function buildMessagesForQuery(sessionId: string, config: ContextConfig, encodedProjectPath: string): EnrichedMessage[] {
  ...
  enriched = applyL1(enriched, config, sessionId, encodedProjectPath);
...
function applyL1(
  ...
  return enriched.map((e) => {
    ...
    return persistAndShrink(e, encodedProjectPath, sessionId, toolCallId, config);
  });
}
...
function persistAndShrink(
  ...
  const { path } = persistToolResult(encodedProjectPath, sessionId, toolCallId, e.message.content);
