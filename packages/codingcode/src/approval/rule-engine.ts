...
export function createRuleEngine(initialRules: PermissionRule[] = []): RuleEngine {
  ...
  function evaluate(tool: string, input: Record<string, unknown>): ApprovalDecision | null {
    ...
    const sorted = Array.from(rules.values()).sort(
    ...
    for (const rule of sorted) {
      ...
      switch (action) {
        case 'deny':
          return { type: 'deny', reason: rule.reason ?? `Denied by rule: ${rule.id}`, source: `rule:${rule.id}` };
        case 'allow':
  ...
  return {
    addRule: (rule: PermissionRule) => { rules.set(rule.id, rule); },
