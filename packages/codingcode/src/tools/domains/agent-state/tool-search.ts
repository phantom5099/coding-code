...
let svc: ToolSearchApi | null = null;
export function bindToolSearchService(impl: ToolSearchApi): void { svc = impl; }
...
export const toolSearchTool: ToolDefinition = {
  ...
  execute: async (args, ctx) => {
    ...
    if (!svc) throw new AgentError('TOOL_EXECUTION_FAILED', 'tool_search service not bound');
