...
describe('HookService 鈥?Decision Handlers', () => {
    ...
    const result = await run(
      Effect.gen(function* () {
        ...
        yield* hooks.registerDecision(
          'tool.approval.pre',
          () => ({ decision: 'deny' as const, reason: 'Second handler' }),
          { priority: 20 }
        );
        yield* hooks.registerDecision('tool.approval.pre', () => ({ decision: 'allow' as const }), {
          priority: 30,
        });
  ...
  it('should separate observer and decision handlers', async () => {
    ...
    const result = await run(
      Effect.gen(function* () {
        ...
        yield* hooks.register('tool.approval.pre', () => {
          calls.push('observer');
        });
        yield* hooks.registerDecision(
          'tool.approval.pre',
  ...
  it('should unregister decision handler via returned function', async () => {
    const result = await run(
      Effect.gen(function* () {
        const hooks = yield* HookService;
        const unregister = yield* hooks.registerDecision('tool.approval.pre', () => ({
