...
describe('buildMessagesFromEvents', () => {
  ...
  it('filters assistant with partially resolved tool_calls and their orphaned tool results', () => {
    const events: SessionEvent[] = [
      ...
      { type: 'user', turnId: 2, uuid: 'u2', content: 'step 2', timestamp: new Date().toISOString() },
      { type: 'assistant', turnId: 2, uuid: 'a2', content: 'done', toolCalls: [], model: 'test', timestamp: new Date().toISOString() },
  ...
  it('removes assistant when summary hides its tool_results but not the assistant itself', () => {
    const events: SessionEvent[] = [
      ...
      { type: 'summary', uuid: 's1', replaces: ['t1'], summaryText: '[compacted]', method: 'prune', timestamp: new Date().toISOString() },
    ...
    const assistantContents = messages.filter((m) => m.role === 'assistant').map((m) => (m as any).content);
    expect(assistantContents).toEqual(['done']);
    // No tool messages should remain
    expect(messages.filter((m) => m.role === 'tool')).toHaveLength(0);
    // Summary should remain as system
    expect(messages.filter((m) => m.role === 'system').map((m) => m.content)).toContain('[compacted]');
  });
  ...
  it('does not merge adjacent tool messages', () => {
    const events: SessionEvent[] = [
      ...
      { type: 'assistant', turnId: 1, uuid: 'a1', content: 'ok', toolCalls: [{ id: 'tc1', name: 'bash', arguments: {} }, { id: 'tc2', name: 'bash', arguments: {} }], model: 'test', timestamp: new Date().toISOString() },
      { type: 'tool_result', turnId: 1, uuid: 't1', parentUuid: 'a1', toolName: 'bash', toolCallId: 'tc1', output: 'out1', timestamp: new Date().toISOString(), tokenCount: 10 },
      { type: 'tool_result', turnId: 1, uuid: 't2', parentUuid: 'a1', toolName: 'bash', toolCallId: 'tc2', output: 'out2', timestamp: new Date().toISOString(), tokenCount: 10 },
    ];
    const messages = buildMessagesFromEvents(events);
