...
import { buildMessages, applyVisibilityEvents, readUIHistory } from '../../src/session/store.js';
...
describe('applyVisibilityEvents', () => {
  it('marks rollback-hidden events', () => {
    ...
    try {
      const events = [
        ...
        { type: 'assistant' as const, turnId: 2, uuid: 'a2', content: 'bye', toolCalls: [], model: 't', timestamp: new Date().toISOString() },
        { type: 'hide' as const, uuid: 'h1', kind: 'rollback' as const, throughTurnId: 1, reason: 'test', timestamp: new Date().toISOString() },
      ];
  ...
  it('unhide restores rollback-hidden events', () => {
    const events = [
      ...
      { type: 'assistant' as const, turnId: 2, uuid: 'a2', content: 'bye', toolCalls: [], model: 't', timestamp: new Date().toISOString() },
      { type: 'hide' as const, uuid: 'h1', kind: 'rollback' as const, throughTurnId: 1, reason: 'test', timestamp: new Date().toISOString() },
      { type: 'unhide' as const, uuid: 'uh1', targetHideUuid: 'h1', timestamp: new Date().toISOString() },
    ];
    const hidden = applyVisibilityEvents(events);
...
describe('buildMessages with visibility filtering', () => {
  it('visible turns match after rollback', () => {
    ...
    const fx = makeFixture(sessionId, slug, [
      { type: 'hide', uuid: randomUUID(), kind: 'rollback', throughTurnId: 1, reason: 'test', timestamp: new Date().toISOString() },
    ]);
    try {
  ...
  it('messages after rollback and unhide match original', () => {
    ...
    try {
      ...
      appendFileSync(tp, JSON.stringify({ type: 'hide', uuid: hideUuid, kind: 'rollback' as const, throughTurnId: 0, reason: 'test', timestamp: new Date().toISOString() }) + '\n', 'utf8');
      appendFileSync(tp, JSON.stringify({ type: 'unhide', uuid: randomUUID(), targetHideUuid: hideUuid, timestamp: new Date().toISOString() }) + '\n', 'utf8');
  ...
  it('message hide can be undone', () => {
    ...
    try {
      ...
      appendFileSync(tp, JSON.stringify({
        type: 'unhide', uuid: randomUUID(), targetHideUuid: 'h-msg', timestamp: new Date().toISOString(),
      }) + '\n', 'utf8');
  ...
  it('rollback hide is NOT undone by undoLastHide simulation', () => {
    ...
    try {
      const events = [
        ...
        { type: 'assistant', turnId: 2, uuid: 'a2', content: 'bye', toolCalls: [], model: 't', timestamp: new Date().toISOString() },
      ];
      const dir = join(PROJECT_BASE, slug, 'sessions');
      mkdirSync(dir, { recursive: true });
      const tp = join(dir, `${sessionId}.jsonl`);
      ...
      appendFileSync(tp, JSON.stringify({
        type: 'hide', uuid: 'h-msg', kind: 'message', targetUuid: 'u1', reason: 'test', timestamp: new Date().toISOString(),
      }) + '\n', 'utf8');

      // Add rollback hide to turn 1 (hides turnId > 1 i.e. turn 2, 'bye')
      appendFileSync(tp, JSON.stringify({
        type: 'hide', uuid: 'h-rollback', kind: 'rollback', throughTurnId: 1, reason: 'test', timestamp: new Date().toISOString(),
      }) + '\n', 'utf8');
  ...
  it('hides turns after rollback', () => {
    ...
    try {
      const events = [
        ...
        { type: 'user', turnId: 2, uuid: 'u2', content: 'bye', timestamp: new Date().toISOString() },
        { type: 'assistant', turnId: 2, uuid: 'a2', content: 'bye', toolCalls: [], model: 't', timestamp: new Date().toISOString() },
        { type: 'hide', uuid: 'h1', kind: 'rollback' as const, throughTurnId: 1, reason: 'test', timestamp: new Date().toISOString() },
      ];
      const dir = join(PROJECT_BASE, slug, 'sessions');
      mkdirSync(dir, { recursive: true });
      const tp = join(dir, `${sessionId}.jsonl`);
      ...
      writeFileSync(join(dir, `${sessionId}.index.json`), JSON.stringify({
        ...
        createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
        messageCount: 7, title: 'test', currentTurnId: 2, usage: undefined, permissionMode: 'default',
      }));
