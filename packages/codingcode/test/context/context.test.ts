...
function makeMockSessionLayer(state: any) {
  return Layer.succeed(
    ...
    SessionService.of({
      ...
      readMessages: () =>
        Effect.succeed(
          state.sessionId === 'full-flow' ? [{ role: 'user', content: 'message one' }] : []
        ),
...
describe('ContextService', () => {
  it('should retain context when sendMessage and resumeSession run in separate Effect scopes', async () => {
    ...
    // Step 1: send message in one Effect scope
    {
      ...
      const { stream: gen, sessionId } = (await Effect.runPromise(
        (program as any).pipe(Effect.provide(fullLayer) as any)
      )) as any;
      sid1 = sessionId;
    ...
    const history3 = (await Effect.runPromise(
      g3.pipe(Effect.provide(mockSessionLayer) as any)
    )) as any[];
