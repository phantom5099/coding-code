...
describe('historyToUIMessages', () => {
  ...
  it('should convert tool_result events to UIMessage with toolName', () => {
    const history = [
      { type: 'tool_result', uuid: 't1', output: 'result', timestamp: '2025-01-01T00:00:00.000Z', toolName: 'read' },
    ];
    const result = historyToUIMessages(history);
    expect(result).toHaveLength(1);
    expect(result[0]).toMatchObject({
  ...
  it('should skip session_meta, role_switch, and compact_boundary events', () => {
    const history = [
      { type: 'session_meta', uuid: 'm1', sessionId: 's1', projectSlug: 'test', cwd: '/', model: 'm', role: 'coder', createdAt: '', version: '1' },
      { type: 'user', uuid: 'u1', content: 'hello', timestamp: '2025-01-01T00:00:00.000Z' },
      ...
      { type: 'compact_boundary', uuid: 'c1', summary: '...', replacedRange: [0, 1], messageCount: 1, timestamp: '' },
    ];
    const result = historyToUIMessages(history);
  ...
  it('should handle conversation with interleaved tool calls', () => {
    const history = [
      ...
      { type: 'assistant', uuid: 'a1', content: 'let me read that', timestamp: '2025-01-01T00:00:02.000Z', model: 'test-model', toolCalls: [{ id: 'tc1', name: 'read', arguments: '{}' }] },
      { type: 'tool_result', uuid: 't1', content: undefined, output: 'file contents here', timestamp: '2025-01-01T00:00:03.000Z', toolName: 'read', parentUuid: 'a1', toolCallId: 'tc1' },
      { type: 'assistant', uuid: 'a2', content: 'the file contains...', timestamp: '2025-01-01T00:00:04.000Z', model: 'test-model', toolCalls: [] },
    ];
    const result = historyToUIMessages(history);
