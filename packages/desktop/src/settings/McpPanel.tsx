...
const EMPTY_FORM: McpForm = {
  ...
  url: '', headers: '', env: '', concurrency: 3, autoReconnect: true,
}
...
export default function McpPanel() {
  ...
  const saveForm = async () => {
    ...
    if (form.transport === 'stdio') {
      ...
      if (form.args.trim()) server.args = form.args.split(',').map(s => s.trim()).filter(Boolean)
    } else {
      server.url = form.url
      if (form.headers.trim()) {
        server.headers = Object.fromEntries(
          form.headers.split('\n').map(l => l.split(/:\s*/)).filter(a => a[0]),
        )
      }
    }
    if (form.env.trim()) {
...
function FormCard({ form, setForm, onSave, onCancel, inputCls, labelCls, btnPrimary, btnCancel }: {
  form: McpForm
    ...
    <div className="px-4 py-3.5 rounded-xl bg-[#1a1a1a] border border-[#569cd6]/30 space-y-3">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-3 py-1.5 rounded text-[13px] font-mono transition-colors ${
              form.transport === 'stdio' ? 'bg-[#1a3a5c] text-[#569cd6]' : 'bg-[#2a2a2a] text-[#888]'
            }`}>
            stdio
          </button>
          <button
      ...
      {form.transport === 'stdio' ? (
        ...
        <>
          <div>
            ...
            <input className={inputCls} value={form.url} title="URL"
              onChange={e => setForm({ ...form, url: e.target.value })} />
          </div>
          <div>
            <div className={labelCls}>请求头 (每行 key: value)</div>
            <textarea className={`${inputCls} h-16`} value={form.headers} title="请求头"
      ...
      <div className="flex gap-4">
        <div className="flex-1">
          ...
          <input type="number" min={1} className={inputCls} value={form.concurrency} title="并发数"
