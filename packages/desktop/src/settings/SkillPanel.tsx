...
export default function SkillPanel({ global: isGlobal }: { global?: boolean }) {
  ...
  if (skills.length === 0)
    return (
      <div className="px-6 py-8 text-[14px] text-[var(--text-disabled)] text-center">
        未找到 Skill
      </div>
    );
      ...
      <div className="space-y-3">
        {skills.map((skill) => (
          ...
          >
            ...
            {skill.description && (
              <div className="text-[13px] text-[var(--text-placeholder)] mt-1">
                {skill.description}
              </div>
            )}
            <div className="text-[11px] text-[var(--text-disabled)] mt-1 truncate">
              {skill.skillPath}
            </div>
          </div>
        ))}
