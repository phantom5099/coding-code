...
export default function ToolCallCard({ item, threadId, onApprove, onReject }: ToolCallCardProps) {
  ...
  return (
    <div
      className={`my-2 rounded border ${isRejected ? 'border-[#444] opacity-50' : 'border-[#3c3c3c]'} bg-[#252526] overflow-hidden`}
    >
      <div className="flex items-center gap-2 px-3 py-2 bg-[#2d2d2d]">
        {hasArgs ? (
          ...
          >
            <span
              className={`transition-transform text-[10px] text-[#555] ${argsOpen ? 'rotate-90' : ''}`}
            >
              ▶
            </span>
            <span
              className={`font-mono text-sm font-medium ${isRejected ? 'line-through text-[#666]' : 'text-[#dcdcaa]'}`}
            >
              {item.name}
            </span>
          </button>
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          {item.status === 'approved' && <span className="text-xs text-[#4ec9b0]">✓ 已完成</span>}
          {item.status === 'rejected' && <span className="text-xs text-[#666]">✗ 已拒绝</span>}
        </div>
      </div>
      {hasArgs && argsOpen && (
