...
function streamChunkToItem(
  ...
  switch (event.type) {
    ...
    case 'tool_result':
      return { id: 'rand', type: 'tool_result', callId: event.id, name: event.name, output: event.output, exitCode: event.ok ? 0 : 1 }
    case 'tool_denied':
...
describe('streamChunkToItem after StreamChunk refactor', () => {
  it('maps text chunk to partial assistant message', () => {
    ...
    expect(item).toEqual({
      id: 'a1', type: 'message', role: 'assistant', content: 'hello', partial: true,
    })
  })
  ...
  it('maps message chunk to non-partial assistant message', () => {
    ...
    expect(item).toEqual({
      id: 'a1', type: 'message', role: 'assistant', content: 'full text', partial: false,
    })
  })
  ...
  it('maps tool_start to running tool_call', () => {
    ...
    expect(item).toEqual({
      id: 'tc1', type: 'tool_call', name: 'bash', args: { command: 'ls' }, status: 'running',
    })
  })
  ...
  it('maps approval_request to pending tool_call', () => {
    ...
    expect(item).toEqual({
      id: 'apr1', type: 'tool_call', name: 'write_file', args: { path: 'x' }, status: 'pending',
    })
  })
  ...
  it('maps tool_result to tool_result item', () => {
    ...
    expect(item).toMatchObject({
      type: 'tool_result', callId: 'tc1', name: 'bash', output: 'ok', exitCode: 0,
    })
  })
