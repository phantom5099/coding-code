...
describe('text selection behavior', () => {
  ...
  describe('TitleBar.tsx', () => {
    it('title bar should have select-none', () => {
      const barClass = classNamesFromSource('TitleBar.tsx').find((c) =>
        c.includes('shrink-0') && c.includes('flex items-center')
      )
  ...
  describe('ProjectStrip.tsx', () => {
    it('project strip should have select-none', () => {
      const stripClass = classNamesFromSource('agent/ProjectStrip.tsx').find((c) =>
        c.includes('w-12') && c.includes('shrink-0')
      )
  ...
  describe('MessageStream.tsx', () => {
    ...
    it('message list containers should have select-text', () => {
      ...
      const hasSelectTextVirtuoso = anyClassContains('agent/MessageStream.tsx', 'flex-1 select-text')
      const hasSelectTextDiv = anyClassContains('agent/MessageStream.tsx', 'overflow-y-auto select-text')
