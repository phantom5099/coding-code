...
describe('buildRenderEntries merges tool_call + tool_result globally', () => {
  ...
  it('renders running tool_call without toolResult', () => {
    const turns: Turn[] = [
      {
        id: 't1',
        items: [makeMsg('user', 'q'), makeToolCall('bash', 'running', 'tc-1')],
        status: 'running',
      },
    ];
    const entries = buildRenderEntries(turns);
  ...
  it('does not render tool_result as standalone entry', () => {
    const turns: Turn[] = [
      {
        id: 't1',
        items: [makeToolCall('bash', 'approved', 'tc-1'), makeToolResult('tc-1', 'ok')],
        status: 'completed',
      },
    ];
    const entries = buildRenderEntries(turns);
  ...
  it('unified tool_call has toolResult after approval_request', () => {
    ...
    const turns: Turn[] = [
      {
        id: 't1',
        items: [makeToolCall('write_file', 'pending', 'tc-new'), makeToolResult('tc-new', 'done')],
        status: 'running',
      },
    ];
    const entries = buildRenderEntries(turns);
